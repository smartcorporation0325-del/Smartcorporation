...
export function buildDemoAnalysisForTranscript(
  ...
  const buyingIntent: "low" | "medium" | "high" = askedForCommitment ? "high" : hasBuyingSignal ? "medium" : "low";
  ...
  return {
    ...
    nextActions: [
      {
        actionType: "email",
        actionDescription: "Send follow-up recap and confirm next call time",
        dueDate: null,
        dueTime: null,
        owner: crm.salesRepName ?? "Sales Rep",
        priority: "medium",
        closeStrategy: askedForCommitment
          ? "Confirm the deposit process once the client reviews the recap."
          : "Use the recap to re-open with a direct ask for commitment on the next touchpoint.",
      },
    ],
    followUpAssessment: {
    ...
    whyThisMatters: buildWhyThisMatters({
      buyingIntent,
      dealOpen: crm.dealStatus !== "closed_won" && crm.dealStatus !== "closed_lost",
      hasClearNextStep: askedForCommitment,
      dealAmount: crm.dealAmount,
      missedClose: hasBuyingSignal && !askedForCommitment,
    }),
    followUpMessages: buildFollowUpMessages({
      contactFirstName: crm.contactName?.split(" ")[0] ?? null,
      repName: crm.salesRepName,
      nextActionSummary: askedForCommitment
        ? "I'll follow up shortly to finalize the details we discussed."
        : "I'm sending over the recap from our call — let's find a time to talk through next steps.",
    }),
  };
}
